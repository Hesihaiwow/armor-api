...
CREATE TABLE `test_example` (
  ...
  `type` int(1) DEFAULT NULL COMMENT '正反用例    0:无, 1:正常用例 2:异常用例',
  `status` int(1) DEFAULT NULL COMMENT '状态  0:无,1:通过,2:失败 3:阻塞',
  `exam_status` int(1) DEFAULT NULL COMMENT '评审状态   0:无, 1:评审通过, 2:评审失败',
  `create_time` datetime NOT NULL COMMENT '创建日期',
  `create_by` bigint(20) NOT NULL COMMENT '创建人id',
  `create_name` varchar(32) DEFAULT NULL COMMENT '创建人姓名',
  `update_time` datetime NOT NULL COMMENT '更新时间',

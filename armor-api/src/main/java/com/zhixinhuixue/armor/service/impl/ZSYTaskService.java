...
public class ZSYTaskService implements IZSYTaskService {
    ...
    public ZSYResult addTask(TaskReqDTO taskReqDTO, Long origin) {
        ...
        // 插入任务标签
        if (!CollectionUtils.isEmpty(taskReqDTO.getTags())) {
    ...
    public ZSYResult modifyTask(Long taskId, TaskReqDTO taskReqDTO) {
        ...
        if (taskTemp == null) {
            ...
            throw new ZSYServiceException(String.format("无法找到任务,id:{%s}", taskId));
        }
        ...
        Task task = new Task();
        if (userRole != ZSYUserRole.ADMINISTRATOR.getValue() && taskTemp.getType() == 2) {
        ...
        if (taskTempBo.getTaskUsers().size() != taskReqDTO.getTaskUsers().size()) {
            ...
            taskTempBo.getTaskUsers().forEach(taskUser -> {
                taskReqDTO.getTaskUsers().forEach(taskUserReq -> {
                    if (taskUser.getUserId().equals(taskUserReq.getUserId()) && !taskUser.getTaskHours().equals(taskUserReq.getTaskHours())) {
                        task.setExamine(ZSYTaskExamine.NOTEXAM.getValue());
                    }
                });
            });
        ...
        List<TaskFunctionReqDTO> functionReqDTOS = taskReqDTO.getFunctionReqDTOS();
        if (!CollectionUtils.isEmpty(functionReqDTOS)) {
            if (!CollectionUtils.isEmpty(functionList) && functionList.size() > functionReqDTOS.size()) {
        ...
        if (!CollectionUtils.isEmpty(taskReqDTO.getTaskUsers())) {
            ...
            taskReqDTO.getTaskUsers().forEach(user -> {
                ...
                if (taskReqDTO.getTaskType() == ZSYTaskType.PRIVATE_TASK.getValue()) {
                    ...
                    if (CollectionUtils.isEmpty(user.getUserWeeks())) {
            ...
            deleteTaskUserIds.forEach(deleteTaskUserId -> {
        ...
        if (taskReqDTO.getTaskType() == ZSYTaskType.PRIVATE_TASK.getValue() && taskTemp.getStatus() == ZSYTaskStatus.FINISHED.getValue()) {
            taskTemp.getTaskUsers().forEach(userOld -> {
                ...
                userBO.setIntegral(currentIntegral.subtract(BigDecimal.valueOf(userOld.getTaskHours())));
            ...
            taskReqDTO.getTaskUsers().forEach(user -> {
                ...
                userIntegral.setIntegral(BigDecimal.valueOf(user.getTaskHours()));
    ...
    public ZSYResult auditTask(Long taskId, Integer level, Long userId, Integer auditStatus) {
        ...
        if (taskTemp.getReviewStatus() != null && (taskTemp.getReviewStatus() == ZSYReviewStatus.REJECT.getValue()
            ...
            logger.warn("该任务已被审核,状态:{},id:{}", ZSYReviewStatus.getName(auditStatus), taskId);
    ...
    public ZSYResult completePrivateTask(TaskCompleteReqDTO taskCompleteReqDTO) {
        ...
        if (taskTemp == null) {
            throw new ZSYServiceException(String.format("找不到任务,id:{%s}", taskCompleteReqDTO.getTaskId()));
        }
        TaskUser taskUserTemp = taskUserMapper.selectByPrimaryKey(taskCompleteReqDTO.getTaskUserId());
        if (taskUserTemp == null) {
            throw new ZSYServiceException(String.format("找不到任务用户,id:{%s}", taskCompleteReqDTO.getTaskUserId()));
        }
    ...
    public ZSYResult completePublicTask(TaskCompleteReqDTO taskCompleteReqDTO) {
        ...
        Task taskTemp = taskMapper.selectByPrimaryKey(taskCompleteReqDTO.getTaskId());
        if (taskTemp == null) {
            throw new ZSYServiceException(String.format("无法找到任务,id:{%s}", taskCompleteReqDTO.getTaskId()));
        }
        TaskUser taskUserTemp = taskUserMapper.selectByPrimaryKey(taskCompleteReqDTO.getTaskUserId());
        if (taskUserTemp == null) {
            throw new ZSYServiceException(String.format("无法找到任务阶段,id:{%s}", taskCompleteReqDTO.getTaskId()));
        }
    ...
    public ZSYResult<TaskDetailResDTO> getTaskDetail(Long taskId) {
        ...
        taskDetailBO.getTaskTags().forEach(tag -> {
        ...
        taskDetailBO.getTaskUsers().forEach(taskUserBO -> {
            ...
            if (!CollectionUtils.isEmpty(evaluationBOS1)) {
            ...
            taskUserBO.getTaskComments().forEach(taskCommentBO -> {
            ...
            taskUserBO.getUserWeeks().forEach(userWeekBO -> {
            ...
            if (!CollectionUtils.isEmpty(taskBOS)) {
                taskBOS.forEach(taskBO -> {
                    ...
                    taskBO.getTaskUsers().forEach(taskUser -> {
            ...
            if (!CollectionUtils.isEmpty(evaluationBOS) && ZSYUserRole.ADMINISTRATOR.getValue() == ZSYTokenRequestContext.get().getUserRole()) {
    ...
    public ZSYResult<List<TaskTestResDTO>> getTestingTask(Long userId) {
        ...
        if (!CollectionUtils.isEmpty(taskBOS)) {
            taskBOS.forEach(taskTestBO -> {
                ...
                double value = BigDecimal.valueOf(hor).setScale(0, BigDecimal.ROUND_HALF_UP).doubleValue();
    ...
    public ZSYResult addComment(CommentReqDTO commentReqDTO) {
        ...
        if (userIds.size() > 1) {
            int sum = commentedNum.stream().mapToInt(Long::intValue).sum();
        ...
        if (CollectionUtils.isEmpty(commentReqDTO.getComments())) {
            throw new ZSYServiceException("评价不能为空");
        }
        List<TaskComment> commentList = new ArrayList<>();
        commentReqDTO.getComments().forEach(commentSalveDTO -> {
        ...
        if (commentCompleted) {
            ...
            taskDetailBO.getTaskUsers().forEach(taskUserBO -> {
                ...
                BigDecimal integral = BigDecimal.valueOf(taskUserBO.getTaskHours())
                        .multiply(BigDecimal.valueOf(average.getAsDouble()))
    ...
    public PageInfo<TaskListResDTO> getTaskListPage(TaskListReqDTO taskListReqDTO) {
        ...
        taskListBOS.forEach(taskListBO -> {
            ...
            List<TaskTagResDTO> taskTagResDTOS = new ArrayList<>();
            taskListBO.getTags().forEach(tag -> {
                TaskTagResDTO taskTagResDTO = new TaskTagResDTO();
                taskTagResDTO.setColor(tag.getColor());
                taskTagResDTO.setName(tag.getName());
                taskTagResDTO.setColorValue(ZSYTagColor.getName(Integer.parseInt(tag.getColor())));
        ...
        logger.info("准备返回值耗时: {}ms", (time3 - time2));
    ...
    public ZSYResult<List<TaskDetailBO>> getAllWaitComment(Long userId) {
        ...
        for (TaskDetailBO taskBO : taskBOS) {
            ...
            for (TaskUserBO taskUserBO : collect) {
                if (hasNext) {
                    if (!CollectionUtils.isEmpty(taskUserBO.getTaskComments())) {
    ...
    public ZSYResult<List<TaskDetailBO>> getCommented(Long userId, Integer page) {
        startPage(page, ZSYConstants.PAGE_SIZE_WAIT);
    ...
    public PageInfo<TaskLogResDTO> getTaskLog(Long taskId, int pageNum) {
        startPage(pageNum, ZSYConstants.PAGE_SIZE);
    ...
    public void missionModified(TaskDetailBO taskTemp, TaskReqDTO taskReqDTO) {
        ...
        if (!CollectionUtils.isEmpty(joiners)) {
            joiners.forEach(userId -> {
                ...
                if (taskReqDTO.getTaskType() != null && taskReqDTO.getTaskType().equals(taskTemp.getType())) {
                ...
                if (taskReqDTO.getPriority() != null && taskReqDTO.getPriority().equals(taskTemp.getPriority())) {
                    String str = "优先级由( " + ZSYTaskPriority.getName(taskTemp.getPriority()) + " ) --> ( " + ZSYTaskPriority.getName(taskReqDTO.getPriority()) + " );\n";
                    sb.append(str);
                }
                if (taskReqDTO.getFacility() != null && taskReqDTO.getFacility().equals(taskTemp.getFacility())) {
                ...
                if ((!CollectionUtils.isEmpty(taskReqDTO.getTaskUsers()))) {
                    ...
                    if (CollectionUtils.isEmpty(taskTemp.getTaskUsers())) {
                        ...
                        taskTemp.getTaskUsers().forEach(taskUserBO -> {
                            //过滤出相同的taskUser
                        ...
                        taskReqDTO.getTaskUsers().forEach(taskUserReqDTO -> {
                            //获取相同的部分
    ...
    @Transactional
    public void readAll() {
        notificationMapper.updateNoticeByUser(ZSYTokenRequestContext.get().getUserId(), new Date());
    }
    ...
    public void noticeDelayMasterTaskPrincipal() {
        ...
        if ("nothing".equals(smsConfig.getApi())) {
            logger.info(smsConfig.getApi());
        } else {
            taskMap.entrySet().forEach(longStringEntry -> {
    ...
    private String sendMessage(String phone, String templateId, String templateJson) {
        ...
        return ZSYOKHttpHelper.post(ZSYOKHttpHelper.componentUrl(smsConfig.getApi(), params));
    }
    ...
    public void noticeDelaySonTaskPrincipal() {
        ...
        if ("nothing".equals(smsConfig.getApi())) {
            ...
            taskMap.entrySet().forEach(entrySet -> {
    ...
    public void setTestingTask(TestingTaskReqDTO testingTask) {
        ...
        bo.getTaskUsers().forEach(taskUserBO -> {
            ...
            userIntegral.setTaskId(taskUserBO.getTaskId());
            userIntegral.setUserId(taskUserBO.getUserId());
            double integral = BigDecimal.valueOf(taskUserBO.getTaskHours() * testingTask.getPercent() * 0.01).setScale(0, BigDecimal.ROUND_HALF_UP).doubleValue();
